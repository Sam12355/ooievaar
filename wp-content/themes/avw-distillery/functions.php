...
/**
 * CONTACT QUERIES: Custom Post Type (admin-only)
 */
function avw_register_query_cpt() {
    register_post_type( 'avw_query', array(
        'labels' => array(
            'name'               => 'Contact Berichten',
            'singular_name'      => 'Bericht',
            'all_items'          => 'Alle Berichten',
...
add_action( 'init', 'avw_register_query_cpt' );

/**
 * CONTACT QUERIES: Custom admin columns
 */
function avw_query_columns( $cols ) {
    return array(
        'cb'            => $cols['cb'],
        'title'         => 'Naam',
        'query_email'   => 'E-mail',
        'query_subject' => 'Onderwerp',
        'query_read'    => 'Status',
        'date'          => 'Datum',
    );
}
add_filter( 'manage_avw_query_posts_columns', 'avw_query_columns' );

function avw_query_column_content( $col, $post_id ) {
    if ( $col === 'query_email' ) {
        $email = get_post_meta( $post_id, '_query_email', true );
        echo '<a href="mailto:' . esc_attr($email) . '">' . esc_html($email) . '</a>';
    }
    if ( $col === 'query_subject' ) {
        echo esc_html( get_post_meta( $post_id, '_query_subject', true ) ?: '—' );
    }
    if ( $col === 'query_read' ) {
        $read = get_post_meta( $post_id, '_query_read', true );
        echo $read
            ? '<span style="color:#2e7d32;font-weight:600;">✓ Gelezen</span>'
            : '<span style="color:#c62828;font-weight:600;">● Nieuw</span>';
    }
}
add_action( 'manage_avw_query_posts_custom_column', 'avw_query_column_content', 10, 2 );

/**
 * CONTACT QUERIES: Show full message in edit screen
 */
function avw_query_message_metabox() {
    add_meta_box( 'avw_query_message', 'Bericht', 'avw_query_message_html', 'avw_query', 'normal', 'high' );
}
add_action( 'add_meta_boxes', 'avw_query_message_metabox' );

function avw_query_message_html( $post ) {
    $email   = get_post_meta( $post->ID, '_query_email',   true );
    $subject = get_post_meta( $post->ID, '_query_subject', true );
    $message = get_post_meta( $post->ID, '_query_message', true );
    update_post_meta( $post->ID, '_query_read', '1' );
    echo '<table style="width:100%;border-collapse:collapse;font-family:sans-serif;font-size:14px;">';
    echo '<tr><td style="padding:8px 12px;background:#f9f9f9;width:120px;font-weight:600;border:1px solid #e0e0e0;">Naam</td><td style="padding:8px 12px;border:1px solid #e0e0e0;">' . esc_html($post->post_title) . '</td></tr>';
    echo '<tr><td style="padding:8px 12px;background:#f9f9f9;font-weight:600;border:1px solid #e0e0e0;">E-mail</td><td style="padding:8px 12px;border:1px solid #e0e0e0;"><a href="mailto:' . esc_attr($email) . '">' . esc_html($email) . '</a></td></tr>';
    echo '<tr><td style="padding:8px 12px;background:#f9f9f9;font-weight:600;border:1px solid #e0e0e0;">Onderwerp</td><td style="padding:8px 12px;border:1px solid #e0e0e0;">' . esc_html($subject) . '</td></tr>';
    echo '<tr><td style="padding:8px 12px;background:#f9f9f9;font-weight:600;border:1px solid #e0e0e0;vertical-align:top;">Bericht</td><td style="padding:8px 12px;border:1px solid #e0e0e0;white-space:pre-wrap;">' . esc_html($message) . '</td></tr>';
    echo '</table>';
}

/**
 * CONTACT QUERIES: AJAX form handler
 */
add_action( 'wp_ajax_avw_contact_submit',        'avw_contact_submit' );
add_action( 'wp_ajax_nopriv_avw_contact_submit', 'avw_contact_submit' );

function avw_contact_submit() {
    check_ajax_referer( 'avw_contact_nonce', 'nonce' );
    ...
    if ( ! $post_id || is_wp_error($post_id) ) {
        wp_send_json_error( 'Er ging iets mis. Probeer het opnieuw.' );
    }

    update_post_meta( $post_id, '_query_email',   $email );
    update_post_meta( $post_id, '_query_subject', $subject );
    update_post_meta( $post_id, '_query_message', $message );
    update_post_meta( $post_id, '_query_read',    '' );

    wp_mail(
        get_option('admin_email'),
        'Nieuw contactbericht: ' . ( $subject ?: 'Geen onderwerp' ),
        "Naam: $name\nE-mail: $email\nOnderwerp: $subject\n\nBericht:\n$message"
    );

    wp_send_json_success( 'Bedankt voor je bericht! We nemen zo snel mogelijk contact op.' );
}
